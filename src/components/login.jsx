...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        setMensaje(response.status >= 500 ? "Error del servidor o base de datos no disponible" : data.message || "Credenciales incorrectas");
        return;
      }
  ...
  const getMensajeClass = () => (!mensaje ? "" : mensaje.includes("exitoso") ? "success" : "error");
